...
- **Compatibilité** : Utilise `pyproject.toml` comme source unique de vérité
...
- Installation des dépendances avec `pip install` (via requirements.txt)
...
├── # requirements.txt         # Supprimé - maintenant dans pyproject.toml
...
| **Compatibilité** | requirements.txt (supprimé) | pyproject.toml |
