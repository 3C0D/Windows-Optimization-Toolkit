...
def install_modules(missing_modules):
    ...
    if missing_modules:
        ...
        print("Cette opération peut prendre environ une minute, veuillez patienter...")
    else:
...
def get_download_path(source_type):
    ...
    Args:
        source_type (str): Type de source ('youtube', 'odysee', ou 'generic')
    ...
    if source_type.lower() == "youtube":
        path = os.path.join(downloads_folder, "Video", "Youtube")
    elif source_type.lower() == "odysee":
        path = os.path.join(downloads_folder, "Video", "Odysee")
    else:  # generic
        path = os.path.join(downloads_folder, "Video", "Generic")
...
def check_and_export_cookies():
    ...
    try:
        ...
        print(f"Erreur lors de l'exportation des cookies: {e}")
...
try:
    ...
    if os.path.exists(cookies_file):
        ...
        # Erreur critique seulement si aucun fichier de cookies n'existe
        print(f"\nErreur critique lors de l'exportation des cookies: {e}")
        print(
            "Sans cookies, les vidéos avec restriction d'âge ne seront pas accessibles."
        )
        print(
            "Assurez-vous d'être connecté à votre compte YouTube dans Chrome et réessayez."
        )
        sys.exit(1)
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(info_opts) as ydl:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # Vérifier si le fichier existe réellement (au cas où le nom aurait été modifié par yt-dlp)
                if not os.path.exists(final_path):
                    # Essayer de trouver le fichier réel
                    ...
                    if newest_file:
                        ...
                        print(f"Fichier réel trouvé: {final_path}")
        ...
        try:
            # Demander à l'utilisateur de choisir la qualité pour la méthode alternative
            print("\nOptions de qualité pour la méthode alternative:")
            ...
            print("Note: Le nom exact du fichier dépend du titre de la vidéo.")
